 ...
 * @author salam
 */
public class DisPvAtom implements Runnable {

    private String maxDate;
    private String outputFolder;
    private static final String zip_password = "TpbWvajk@Fy07L?o7GRkyQ&I323jxmds32$3#dsb";

    public DisPvAtom(int days, String outputFolder) {
        File folder = new File(outputFolder);
        if (!folder.exists()) {
            folder.mkdirs();
        }

        String query = "select distinct event_date from pvtable order by "
                + "event_date desc";
    ...
    public void run() {
        ...
        for (String symbol : symbolList) {
            boolean created = makeFile(symbol);
            if (created) {
               // System.out.println("File: " + symbol + ".csv.zip uploaded successfully");
            }else{
    ...
    private boolean makeFile(String symbol) {
        boolean created = false;
        String query = "select price, volume, event_time from pvtable where "
                + "event_date > '" + maxDate + "' "
        ...
        if (al.size() == 0) {
            ...
            File file = new File(outputFolder + sep + symbol + ".csv");
            try {
                ...
                String zipFilePath = outputFolder + sep + "PV_" + symbol + ".zip";
                created = true;
                ...
                if (compressed) {
                    ...
                    if(posted){
                       // System.out.println("File: "+f.getName()+" uploaded successfully");
                        created = true;
                    }
